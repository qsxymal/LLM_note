...
调用 [[common_ops#save_partial_states|`save_partial_states`]] 函数（来自 `vllm.models.deepseek_v4.common.ops.save_partial_states`）：
```python
save_partial_states(
...
if current_platform.is_cuda():
    if self.head_dim == 512:
        ...
        compress_norm_rope_store_fn = [[SparseAttnCompress|compress_norm_rope_store_cutedsl]]
    else:
        compress_norm_rope_store_fn = [[DeepseekV4_KVCache_Ops#FusedCompressQuantCache|compress_norm_rope_store_triton]]
else:
    compress_norm_rope_store_fn = [[DeepseekV4_KVCache_Ops#FusedCompressQuantCache|compress_norm_rope_store_triton]]
```
